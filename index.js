...
const { validateAlgorithm, validateKey } = require("./src/validators");
const { LetterLetter, LetterNumber } = require("./src/encrypters");

const { charset, algorithm, encrypt, decrypt, key } = parser();
...
(async () => {
    ...
    const message = await (async () => {
        try {
            return await fs.promises.readFile(path.resolve(process.cwd(), encrypt || decrypt), "utf-8");
        } catch (err) {
            throw err;
        }
    })();
